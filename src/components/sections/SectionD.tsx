...
export default function SectionD() {
    ...
    return (
        <section className="min-h-screen flex flex-col justify-center py-20 bg-transparent w-full overflow-hidden relative">
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="flex flex-col gap-12 mb-16 md:relative md:min-h-[70vh]">
                    {/* Text Content — 큰 화면에서 좌측 상단 */}
                    <div className="xl:w-3/5">
                        <h2 className="text-5xl md:text-6xl xl:text-[5.625rem] font-bold text-white mb-6">Our Philosophy</h2>
                        <h3 className="text-[0.8rem] md:text-xl xl:text-3xl font-semibold text-gray-200 mb-4">우리는 피부위에 예술을 새깁니다.</h3>
                        <p className="text-[0.6rem] md:text-base xl:text-2xl text-gray-400 leading-relaxed mb-6 whitespace-pre-line">
                            각 작품은 단순한 그림이 아닌, 당신의 이야기를 담은 영원한 캔버스입니다.<br />
                            정밀함과 창의성의 균형, 그리고 고객과의 깊은 소통을 통해 세상에 단 하나뿐인 작품을 만듭니다.
                        </p>
                        <Link href="/about">
                            <button className="px-3 py-1.5 md:px-6 md:py-3 xl:px-8 xl:py-4 text-[0.5rem] md:text-base xl:text-2xl bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded transition backdrop-blur-sm">
                                Detail
                            </button>
                        </Link>
                    </div>
                    ...
                    <div className="flex flex-col items-center w-full mt-8 md:mt-0 xl:absolute xl:bottom-0 xl:right-0 xl:w-auto">
                        {/* Top Card */}
                        <div className="w-[228px] md:w-[330px] shrink-0">
                            <HexCard title={cards[0].title} desc={cards[0].desc} />
                        </div>

                        {/* Bottom Row */}
                        <div className="flex w-full justify-center gap-[3vw] md:gap-6 -mt-[50px] md:-mt-[70px]">
                            {cards.slice(1).map((card, i) => (
                                <div key={i} className="w-[228px] md:w-[330px] shrink-0">
    ...
    return (
        <div className="filter drop-shadow-[0_8px_24px_rgba(0,0,0,0.6)] hover:drop-shadow-[0_16px_40px_rgba(0,0,0,0.8)] hover:-translate-y-2 transition-all duration-300">
            ...
            >
                ...
                {/* ── Frosted Glass Background (guaranteed visible even without backdrop-blur) ── */}
                <div className="absolute inset-0 z-[1] bg-gradient-to-b from-slate-700/60 via-slate-800/70 to-slate-900/80" />
