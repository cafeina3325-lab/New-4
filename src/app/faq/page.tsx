    ...
    return (
        <main className="pt-[120px] min-h-screen bg-[#1C1310] text-[#D6C7B5] pb-20">
            ...
            <section className="container mx-auto px-6 mb-16 text-center">
                <h1 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight text-[#D6C7B5] break-keep">명확한 기준, 투명한 원칙 (Clear Protocol)</h1>
                <p className="text-lg md:text-xl text-[#D4B483] max-w-4xl mx-auto leading-relaxed word-keep-all opacity-80 font-light break-keep">
            ...
            <section className="container mx-auto px-6 mb-12 max-w-2xl">
                <div className="relative">
                    <input
                        ...
                        className="w-full py-4 px-6 pl-12 bg-white/5 border border-white/5 rounded-lg focus:outline-none focus:border-[#B69676]/30 focus:ring-1 focus:ring-[#B69676]/30 transition-all text-[#D6C7B5] placeholder-[#D4B483]/40 backdrop-blur-sm"
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                    {/* Search Icon */}
                    <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#D4B483]/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                </div>
                <p className="text-sm text-[#D4B483]/40 mt-2 text-center font-light">
            ...
            <section className="container mx-auto px-6 max-w-5xl flex flex-col md:flex-row gap-12 items-start relative">
                {/* Sticky Nav */}
                <aside className="md:w-1/4 md:sticky md:top-32 w-full overflow-x-auto md:overflow-visible z-10 bg-[#1C1310]/80 backdrop-blur-md md:bg-transparent pb-2 md:pb-0">
                    <div className="flex md:flex-col gap-2">
                        {categories.map((cat) => (
                            <button
                                key={cat}
                                onClick={() => setActiveCategory(cat)}
                                className={`px-4 py-2 text-sm font-bold text-left rounded-lg transition-all whitespace-nowrap ${activeCategory === cat
                                    ? "bg-[#B69676] text-[#1C1310] shadow-lg transform scale-[1.05]"
                                    : "bg-white/5 text-[#D4B483]/60 hover:bg-[#B69676]/10 hover:text-[#D6C7B5]"
                                    }`}
                            >
                                {cat}
                            </button>
                ...
                <div className="md:w-3/4 w-full space-y-4">
                    {filteredFAQ.length > 0 ? (
                        filteredFAQ.map((item, index) => (
                            <div
                                key={index}
                                className={`glass-panel rounded-lg overflow-hidden transition-all duration-300 border-white/5 bg-[#1C1310]/50 ${openIndex === index ? 'shadow-lg border-[#B69676]/30 bg-[#1C1310]/80' : 'hover:border-[#B69676]/20 hover:bg-[#B69676]/5'}`}
                            >
                                <button
                                ...
                                >
                                    <div className="flex-1 pr-8">
                                        <span className="text-xs font-bold text-[#B69676]/50 uppercase tracking-wider mb-2 block">{item.category}</span>
                                        <div className="flex items-start gap-2">
                                            <span className="text-[#B69676]/40 font-bold shrink-0 text-lg md:text-xl">Q.</span>
                                            <h3 className={`text-lg md:text-xl font-bold transition-colors break-keep [text-wrap:pretty] ${openIndex === index ? 'text-[#D6C7B5]' : 'text-[#D4B483]/80 group-hover:text-[#D6C7B5]'}`}>
                                                {item.q}
                                            </h3>
                                        </div>
                                    </div>
                                    <div className={`mt-1 transition-transform duration-300 text-[#D4B483]/50 ${openIndex === index ? 'rotate-180 text-[#B69676]' : ''}`}>
                                ...
                                >
                                    <div className="p-6 pt-0 border-t border-white/5">
                                        <div className="flex items-start gap-3 mt-4">
                                            <span className="text-[#B69676]/60 font-bold shrink-0 mt-1">A.</span>
                                            <p className="text-[#D4B483] leading-relaxed whitespace-pre-line word-keep-all font-light opacity-90 break-keep">
                                                {item.a}
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <div className="text-center py-20 text-[#D4B483]/40 glass-panel border-dashed border-white/5 rounded-lg bg-white/5 font-light">
                            검색 결과가 없습니다.
                        </div>
                    )}
                </div>
            ...
            <section className="bg-white/5 border-t border-white/5 py-20 mt-20">
                <div className="container mx-auto px-6 text-center max-w-3xl">
                    <h2 className="text-2xl md:text-3xl font-bold mb-4 text-[#D6C7B5] break-keep">당신만의 고유한 서사에 대해 더 깊은 논의가 필요하십니까?</h2>
                    <p className="text-[#D4B483] mb-8 leading-relaxed word-keep-all opacity-80 font-light break-keep">
                        세부적인 도안 방향성이나 체질에 따른 특이사항 등, 남겨진 질문이 있다면 전담 디렉터에게 문의해 주십시오.
                        프라이빗하고 심도 있는 상담을 도와드립니다.
                    </p>
                    <button
                        onClick={() => setIsContactOpen(true)}
                        className="px-8 py-4 bg-[#B69676] text-[#1C1310] font-bold rounded-lg hover:bg-[#A68666] transition shadow-lg hover:shadow-[#B69676]/20"
                    >
                        1:1 프라이빗 컨시어지 연결
                    </button>
                </div>
